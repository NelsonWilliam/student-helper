...
class JoplinDatabase extends Database {
	...
	async upgradeDatabase(fromVersion) {
		...
		// currentVersionIndex < 0 if for the case where an old version of Student Helperrt used with a newer
		// version of the database, so that migration is not run in this case.
		if (currentVersionIndex < 0) throw new Error('Unknown profile version. Most likely this is an old version of Student Helperrt, while the profile was created by a newer version. Please upgrade Student Helperrt at https://joplin.cozic.net and try again.');
